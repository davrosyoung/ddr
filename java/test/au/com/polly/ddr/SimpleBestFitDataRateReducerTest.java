...
public void testReducingDataToTenIntervals()
{
    ...
    // let's output the reduced dataset to file as reduced.obj
    // -------------------------------------------------------
    File outputFile;
    FileOutputStream fos;
    ObjectOutputStream oos;

    outputFile = new File( "data/reduced.obj" );
    try
    {
        fos = new FileOutputStream( outputFile );
        oos = new ObjectOutputStream( fos );
        oos.writeObject( reducedDataSet );
        oos.close();
        fos.close();
    } catch (IOException e)
    {
        logger.error( "FAILED to write reduced.obj file" );
